...
file2 = "../Измерения/20.12.23/097_cv.cor"

sample = PotentiostatMeasurements(cor_file)
sample2  = PotentiostatMeasurements(file2)
...
s1 = per_cycle * 1
s2 = per_cycle * 11
current1 = list(map(lambda x: 1000 * x / 5, sample.current))
current2 = list(map(lambda x: 1000 * x / 5, sample2.current))
# ax.plot(sample.voltage[:s1], current[:s1], 'c-')
ax.plot(sample.voltage[-per_cycle:], current1[-per_cycle:], 'r-')
ax.plot(sample2.voltage[-per_cycle:], current2[-per_cycle:], 'deepskyblue')
...
plt.legend(["Гладкий слой ITO + 20 нм NiO", "Пористый ITO толщиной 250 нм + 20 нм NiO"], fontsize=18)
...
fig.savefig("../ЦВА 175 + 091", dpi=500)
